		...
		while batch_no*args.batch_size < loaded_data['data_length']:
			...
			if args.model == 'gan':
				...
				_, d_loss, gen, d1, d2, d3 = sess.run([d_optim, loss['d_loss'], outputs['generator']] + check_ts,
													  feed_dict = {input_tensors['t_real_image'] : real_images,
				...
				check_ts = [checks['mean_disc_real_image'], checks['mean_wrong_real_image'], checks['mean_disc_fake_image'],
							checks['z1_corr'], checks['z2_corr']]
				_, d_loss, gen, md_real_image, mw_real_image, md_fake_image, z1, z2 = sess.run([d_optim, loss['d_loss'], outputs['generator']] + check_ts,
										  feed_dict={input_tensors['t_real_image']: real_images,
													 input_tensors['t_wrong_image']: wrong_images,
													 input_tensors['t_real_caption']: caption_vectors,
													 input_tensors['t_z']: z_noise,
			...
			_, g_loss, gen, result = sess.run([g_optim, loss['g_loss'], outputs['generator'], merged],
				feed_dict = {
					input_tensors['t_real_image'] : real_images,
					input_tensors['t_wrong_image'] : wrong_images,
					input_tensors['t_real_caption'] : caption_vectors,
			...
			writer.add_summary(result, i*loaded_data['data_length']+batch_no)
			print("LOSSES", d_loss, g_loss, batch_no, i, len(loaded_data['image_list'])/ args.batch_size)
			batch_no += 1
			if (batch_no % args.save_every) == 0:
				print("Saving Images, Model")
				save_for_vis(args.data_dir, args.model, real_images, gen, image_files)
				save_path = saver.save(sess, "Data/Models/{}/latest_model_{}_temp.ckpt".format(args.model,args.data_set))
		if i%5 == 0:
			save_path = saver.save(sess, "Data/Models/{}/model_after_{}_epoch_{}.ckpt".format(args.model,args.data_set, i))
	writer.close()
...
def save_for_vis(data_dir, model_type, real_images, generated_images, image_files):
	
	shutil.rmtree( join(data_dir, '{}_samples'.format(model_type)) )
	os.makedirs( join(data_dir, '{}_samples'.format(model_type)) )

	for i in range(0, real_images.shape[0]):
		real_image_255 = np.zeros( (64,64,3), dtype=np.uint8)
		real_images_255 = (real_images[i,:,:,:])
		scipy.misc.imsave( join(data_dir, '{}_samples/{}_{}.jpg'.format(model_type, i, image_files[i].split('/')[-1] )) , real_images_255)

		fake_image_255 = np.zeros( (64,64,3), dtype=np.uint8)
		fake_images_255 = (generated_images[i,:,:,:])
		scipy.misc.imsave(join(data_dir, '{}_samples/fake_image_{}.jpg'.format(model_type,i)), fake_images_255)
